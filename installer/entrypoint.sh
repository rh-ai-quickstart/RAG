#!/bin/bash
# ============================================================================
# RAG Quickstart Installer Entrypoint
# ============================================================================
...
write_log_configmap() {
  ...
  local cm_name="rag-installer-log-${job_name}"
  ...
  oc label configmap "$cm_name" \
    ...
    "rag-installer/expires-at=${expires_at}" 2>&1 || true
}
...
write_termination_message() {
  ...
  if [[ -n "$job_name" && "$job_name" != "unknown" ]]; then
    oc annotate job "$job_name" \
      --namespace default \
      --overwrite \
      "rag-installer/termination-message=$message" 2>/dev/null || true
  fi
}
...
case "$ACTION" in
  ...
  STATUS)
    log_status "running" "verifying" "Verifying RAG deployment status..."
    verify_deployment
  ...
  INSTALL)
    ...
    log_status "running" "deploying" "Installing RAG in $INSTALL_MODE mode..."
    deploy_quickstart
  ...
  UNINSTALL_DELETE_ALL)
    log_status "running" "uninstalling" "Removing RAG and all data..."
  ...
  UNINSTALL_KEEP_DATA)
    log_status "running" "uninstalling" "Removing RAG (keeping data volumes)..."
  ...
  UPGRADE)
    log_status "running" "upgrading" "Upgrading RAG deployment..."
    upgrade_quickstart
